...
def calcular_aguinaldo():
    # Cargar los tipos de nómina para el filtro
    tipos_nomina = TipoNomina.query.all()
    # Manejo de formulario POST para iniciar el cálculo
    if request.method == 'POST':
        ...
        try:
            for empleado in empleados_a_procesar:
                ...
                # Si ya existe, saltar al siguiente empleado
                if aguinaldo_existente:
                    nominas_procesadas.append({'nombre': empleado.nombre_completo,
                                                'monto': None,
                                                'estado': 'Ya Existe',
                                                'motivo': 'Aguinaldo ya calculado y guardado para este año.'})
                    continue             
                ...
                if monto_aguinaldo == 0.0 and meses_con_nomina > 0:
                    ...
                    nominas_procesadas.append({'nombre': empleado.nombre_completo,
                                               'monto': 0.0,
                                                'estado': 'Advertencia',
                                                'motivo': f'Cálculo resultó en $0.00. \
                                                     (Meses con Nomina: {meses_con_nomina}). Revisar registros.'})
                    continue
                if monto_aguinaldo == 0.0 and meses_con_nomina == 0:
                    ...
                    nominas_procesadas.append({'nombre': empleado.nombre_completo,
                                               'monto': 0.0,
                                                'estado': 'Advertencia',
                                                'motivo': 'No se encontraron registros de nómina válidos en el período de cálculo.'})
                    continue
